...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  const sidebarClass = cn(
    "w-full md:w-64 lg:w-72 bg-secondary border-r border-neutral-dark flex flex-col",
    "fixed md:static inset-0 z-50 transition-transform duration-300 ease-in-out",
    {
            ...
            <ul className="space-y-1">
              {keywordsData ? (
                keywordsData.map((keyword) => (
                  <li
                    key={keyword.id}
                    className="flex items-center p-3 rounded-md hover:bg-neutral-dark/50 transition-colors"
                  >
                    <span className={cn(
                      "text-sm relative flex items-center pulse-dot",
                      getStatusClass(getKeywordStatus(keyword.keyword))
                    )}>
                      "{keyword.keyword}"
                    </span>
                ...
                <li className="p-3 text-sm opacity-50">Loading keywords...</li>
              )}
            </ul>
          </div>
        </nav>
