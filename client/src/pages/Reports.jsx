...
export default function Reports() {
    var _a = useState("30d"), chartTimeRange = _a[0], setChartTimeRange = _a[1];
    var _b = useState("threats"), reportType = _b[0], setReportType = _b[1];
    var CustomTooltip = function (_a) {
        var active = _a.active, payload = _a.payload, label = _a.label;
        if (active && payload && payload.length) {
            return (<div className="bg-background p-3 border border-neutral-dark rounded-md shadow-md">
          <p className="font-mono text-sm text-white">{"".concat(label)}</p>
          {payload.map(function (entry, index) { return (<p key={index} className="text-xs" style={{ color: entry.color }}>
              {"".concat(entry.name, ": ").concat(entry.value)}
            </p>); })}
        </div>);
        }
        return null;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="border-neutral-dark bg-secondary">
            ...
            <CardContent className="p-5 h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={regionData} cx="50%" cy="50%" innerRadius={70} outerRadius={100} dataKey="value" label={function (_a) {
        var name = _a.name, percent = _a.percent;
        return "".concat(name, " ").concat((percent * 100).toFixed(0), "%");
    }} labelLine={false}>
                    {regionData.map(function (entry, index) { return (<Cell key={"cell-".concat(index)} fill={entry.color}/>); })}
                  </Pie>
                  <Tooltip content={<CustomTooltip />}/>
                </PieChart>
              </ResponsiveContainer>
          ...
          <Card className="border-neutral-dark bg-secondary">
            ...
            <CardContent className="p-5 h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={attackTypeData} cx="50%" cy="50%" innerRadius={70} outerRadius={100} dataKey="value" label={function (_a) {
        var name = _a.name, percent = _a.percent;
        return "".concat(name, " ").concat((percent * 100).toFixed(0), "%");
    }} labelLine={false}>
                    {attackTypeData.map(function (entry, index) { return (<Cell key={"cell-".concat(index)} fill={entry.color}/>); })}
                  </Pie>
                  <Tooltip content={<CustomTooltip />}/>
                </PieChart>
              </ResponsiveContainer>
