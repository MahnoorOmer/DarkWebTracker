...
export default function WebScrapeTools() {
    ...
    var _a = useState("scrape"), activeTab = _a[0], setActiveTab = _a[1];
    var _b = useState(""), sourceName = _b[0], setSourceName = _b[1];
    var _c = useState(""), sourceUrl = _c[0], setSourceUrl = _c[1];
    var _d = useState("15min"), scrapeInterval = _d[0], setScrapeInterval = _d[1];
    var _e = useState(true), proxyEnabled = _e[0], setProxyEnabled = _e[1];
    var _f = useState(true), torRouting = _f[0], setTorRouting = _f[1];
    var _g = useState(true), limitedThreads = _g[0], setLimitedThreads = _g[1];
    var _h = useState(""), mockCommandOutput = _h[0], setMockCommandOutput = _h[1];
    var handleNewSource = function (e) {
        e.preventDefault();
        ...
        toast({
            title: "Source added",
            description: "".concat(sourceName, " has been added to the scraping queue"),
        });
        // Simulate terminal output
        setMockCommandOutput(function (prev) {
            return "".concat(prev, "\n> Adding new source: ").concat(sourceName, "\n> URL: ").concat(sourceUrl, "\n> Config: interval=").concat(scrapeInterval, ", proxy=").concat(proxyEnabled ? "enabled" : "disabled", ", tor=").concat(torRouting ? "enabled" : "disabled", "\n> Source added successfully\n");
        });
        // Reset form
        setSourceName("");
        setSourceUrl("");
    };
    var handleTestScraper = function () {
        // Simulate terminal output for test scraper
        setMockCommandOutput(function (prev) {
            return "".concat(prev, "\n> Testing scraper configuration...\n> Proxy status: ").concat(proxyEnabled ? "Connected" : "Disabled", "\n> Tor routing: ").concat(torRouting ? "Active" : "Disabled", "\n> Anonymity check... PASSED\n> Connectivity test... PASSED\n> Test complete\n");
        });
        toast({
    ...
    return (<Card className="border-neutral-dark bg-secondary">
      ...
      <CardContent className="p-4">
        {activeTab === "scrape" ? (<div className="space-y-4">
            <form onSubmit={handleNewSource}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="source-name" className="text-sm">Source Name</Label>
                  <Input id="source-name" placeholder="e.g., Dark Forum" value={sourceName} onChange={function (e) { return setSourceName(e.target.value); }} className="mt-1 bg-background border-neutral-dark"/>
                </div>
                <div>
                  <Label htmlFor="source-url" className="text-sm">Source URL (onion/i2p)</Label>
                  <Input id="source-url" placeholder="e.g., http://examplexyzabcd.onion" value={sourceUrl} onChange={function (e) { return setSourceUrl(e.target.value); }} className="mt-1 bg-background border-neutral-dark"/>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={torRouting} onCheckedChange={setTorRouting}/>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={limitedThreads} onCheckedChange={setLimitedThreads}/>
                </div>
              </div>
            ...
            <div className="flex items-center p-2 bg-background/30 rounded-md border border-neutral-dark">
              <Input placeholder="Enter command..." className="flex-1 bg-transparent border-none text-xs focus-visible:ring-0 h-6 py-0 font-mono"/>
